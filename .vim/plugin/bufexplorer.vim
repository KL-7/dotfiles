...
function! s:BEGetBufferInfo(bufnr)
  ...
  for buf in split(bufoutput, '\n')
    let bits = split(buf, '"')
" KL-7:    let b = {"attributes": bits[0], "line": substitute(bits[2], '\s*', '', '')}
    let b = {"attributes": substitute(bits[0], '\s\(.*\)\s', '\1', ''), "line": substitute(bits[2], '\s*', '', '')}
...
let s:refreshBufferList = 1
let s:MRU_Exclude_List = ["[BufExplorer]","__MRU_Files__", "__Tag_List__"]
"1}}}
